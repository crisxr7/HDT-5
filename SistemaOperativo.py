...
#SistemaOperativo.py
#programa principal simula la funcion de un cpu, donde el cpu corre cada proceso con una cantidad de memoria ram requerida
...
RANDOM_SEED = 300
NEW_CUSTOMERS = 100  #numero total de procesos
...
#contiene la parte del cpu , donde ejecuta los procesos con sus instrucciones, donde intenta acelerar             
def running(env, name, interval,ram_proceso,cpu_ram_total,instruccion_proceso,tiempo,waiting):
    ...
    while instruccion_proceso > 0:
        siguiente = random.choice(['espera','siguiente']) #paso para escoger el random si se ejecuta el proceso o no
...
cpu = simpy.Resource(env, capacity=3) #crea cpu con un procesador
totalwait = 0
